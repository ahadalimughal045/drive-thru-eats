                  ...
                  {cat.items.map((item: any) => (
                    <div key={item.id} className="bg-white rounded-3xl border border-gray-100 p-4 shadow-sm group hover:shadow-premium transition-all">
                      <div className="aspect-square rounded-2xl bg-gray-50 overflow-hidden mb-4 relative">
                        <img src={item.image && (item.image.startsWith('http') || item.image.startsWith('data:')) ? item.image : `/admin/oimg/${item.image}`} alt={item.name || "Item image"} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-500" />
                    ...
                    <div className="space-y-4">
                      ...
                      {newItem.image ? (
                        <div className="relative w-full aspect-video rounded-[2rem] overflow-hidden group">
                          <img src={newItem.image && (newItem.image.startsWith('http') || newItem.image.startsWith('data:')) ? newItem.image : `/admin/oimg/${newItem.image}`} alt="Upload preview" className="w-full h-full object-cover" />
                          <button 

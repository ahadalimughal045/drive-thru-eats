    ...
    <div className="group relative bg-white rounded-3xl overflow-hidden border border-brand-border hover:border-brand-red/30 transition-all duration-500 shadow-soft hover:shadow-premium hover:-translate-y-2">
      ...
      <div className="relative overflow-hidden aspect-square bg-brand-bg">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          {(item as any).discount > 0 && (
            <span className="bg-green-500 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl shadow-lg border border-white/20">
              {(item as any).discount}% OFF
            </span>
          )}
          {(item as any).tags?.split(',').map((tag: string) => tag.trim() && (
            <span key={tag} className="bg-brand-red text-white text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl shadow-lg border border-white/20">
              {tag.trim()}
            </span>
          ))}
          {inCart && (
            <div className="glass px-2 py-1 rounded-lg text-brand-text text-[10px] font-black animate-bounce-soft">
      ...
      <div className="p-4 flex flex-col h-[165px]">
        <div className="mb-1">
          ...
          <h3 className="text-brand-text font-black text-sm lg:text-[14px] leading-tight group-hover:text-brand-red transition-colors line-clamp-2">
            {item.name}
          </h3>
        ...
        <div className="mt-auto flex items-end justify-between">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
              <span className="text-base font-black text-brand-red tracking-tighter">
